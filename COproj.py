...
def reg_add(register):
    registermap = {
        ...
        't0': '00101', 't1': '00110', 't2': '00111', 't3': '11100', 't4': '11101',
        't5': '11110', 't6': '11111',
...
def s_type(data):
    ...
    opcode, funct3 = opcode_map.get(data[0], (None, None))
    if not opcode:
        print("error: invalid S-type instruction")
        exit()
    imm = binary_convertor(data[3], 12)
    return [imm, reg_add(data[2]), reg_add(data[1]), funct3, opcode]
...
def b_type(data):
    ...
    opcode, funct3 = opcode_map.get(data[0], (None, None))
    if not opcode:
        print("error: invalid B-type instruction")
        exit()
    imm = binary_convertor(data[3], 12)
    return [imm, reg_add(data[2]), reg_add(data[1]), funct3, opcode]
...
def j_type(data):
    ...
    opcode = opcode_map.get(data[0], None)
    if not opcode:
        print("error: invalid J-type instruction")
        exit()
    rd  = reg_add(data[1])
    imm = binary_convertor(data[2], 20)
